...
The OpenAlex dataset is available on [Redivis](https://redivis.com) as well as on KLC at `/kellogg/data/openalex`.

```{note}
OpenAlex is a public catalog that anyone can query via the [OpenAlex API](https://docs.openalex.org/). We list it here because Kellogg Research Support hosts a queryable copy on Redivis and files on KLC at `/kellogg/data/openalex`, so researchers can work with the data without querying the API directly.
```
...
- [OpenAlex White Paper](https://arxiv.org/abs/2205.01833)