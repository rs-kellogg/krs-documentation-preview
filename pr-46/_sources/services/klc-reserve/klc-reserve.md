...
| **Best for** | Development, debugging, and logged production runs | Dedicated resources, GPUs, job arrays |
...
## Current Reserve Node Availability

The table below shows current available CPU cores, GPU cards, and RAM on each KLC Reserve node. Figures refresh about every 20 minutes. For the full live status page, see [live KLC node availability](https://d1p18nmj81zs72.cloudfront.net/klcnodes/klcnodes.html).

<div class="klc-node-status" data-klc-nodes="slurm" role="region" aria-label="KLC Slurm Node Availability">
...
For large in-memory CPU jobs on the dedicated high-memory node, pin the job with `--nodelist=qhimem0501`. See [High-Memory Jobs](highmem-jobs).
...
| Run a large in-memory CPU job | [High-Memory Jobs](highmem-jobs) |
| Need more GPUs than Kellogg nodes provide | [GPU Concepts and Options](/services/klc-reserve/gpu-concepts) |
