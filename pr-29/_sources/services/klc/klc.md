...
The User Guide covers all KLC topics in depth — connecting, storage, file transfer, job scheduling, GPU access, and more.
...
KLC Reserve gives you dedicated GPU and high-core-count nodes through the SLURM job scheduler — for workloads that need more than a shared login node can provide.
...
KLC Reserve uses the same Northwestern NetID, KLC account, and Kellogg file systems as standard KLC login access. Reserve is where you submit SLURM jobs to the `kellogg` partition from any KLC node — including GPU nodes and full-node core counts that are not available on shared login nodes.
