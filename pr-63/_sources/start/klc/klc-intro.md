...
| [High-memory CPU](/services/klc-reserve/highmem-jobs) | Very large in-memory datasets, parallel batch jobs |
...
Once connected, you have a home directory on KLC. Kellogg also provides a separate project directory for research work:
...
```{note}
A Kellogg project directory is not created for you by default. Email [Kellogg Research Support](mailto:rs@kellogg.northwestern.edu) if you want to create a project directory or share a directory with collaborators.
```
...
* **Stay within the 24-core limit** per user across all KLC nodes. For jobs that should survive a disconnect, use `tmux` and save output to a log file — see [Launching Jobs](/services/klc/user-guide/klc-software).
