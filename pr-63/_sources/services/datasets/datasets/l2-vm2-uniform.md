...
| **Update frequency** | Periodic (see Redivis version history; KLC filenames include snapshot dates) |
...
The L2 VM2 Uniform dataset is available on [Redivis](https://redivis.com) under the Northwestern organization and on the [Kellogg Linux Cluster (KLC)](../../klc/user-guide/klc-accessing) as Parquet files under `/kellogg/data/l2/parquet/VM2Uniform/`. Request access through Redivis first; after your application is approved, we enable file-system access on KLC for approved users with a KLC account.

Access on Redivis requires two steps beyond a standard Northwestern Redivis login:
...
After Redivis access is granted, Parquet snapshots are available on KLC at:

```
/kellogg/data/l2/parquet/VM2Uniform/
```

Each state has its own subdirectory named with the two-letter state code (`AK`, `AL`, …, `WY`; `DC` for the District of Columbia). Within each state directory, files follow the naming pattern `VM2Uniform--{ST}--YYYY-MM-DD.parquet`, where `{ST}` is the state code and the date is the snapshot date in ISO format. Sorting filenames lexicographically selects the most recent snapshot.

Example directory layout:

```
/kellogg/data/l2/parquet/VM2Uniform/AK:
  VM2Uniform--AK--2025-12-22.parquet  VM2Uniform--AK--2026-05-08.parquet  VM2Uniform--AK--2026-06-24.parquet

/kellogg/data/l2/parquet/VM2Uniform/AL:
  VM2Uniform--AL--2025-12-19.parquet  VM2Uniform--AL--2026-06-12.parquet
```

If you receive `Permission denied` when listing or reading these paths, file-system access is not yet enabled. Contact [Kellogg Research Support](mailto:rs@kellogg.northwestern.edu).

Run these examples from a KLC session. See [How to Access KLC](../../klc/user-guide/klc-accessing) if you do not yet have an account.
...
BASE = Path("/kellogg/data/l2/parquet/VM2Uniform")


def latest_vm2_uniform(state: str) -> Path:
    state = state.upper()
    candidates = sorted(BASE.glob(f"{state}/VM2Uniform--{state}--*.parquet"))
    if not candidates:
        raise FileNotFoundError(f"No VM2Uniform parquet files found for {state}")
    return candidates[-1]


path = latest_vm2_uniform("IL")
print(path)
...
BASE <- "/kellogg/data/l2/parquet/VM2Uniform"

latest_vm2_uniform <- function(state) {
  state <- toupper(state)
  pattern <- file.path(BASE, state, paste0("VM2Uniform--", state, "--*.parquet"))
  candidates <- sort(Sys.glob(pattern))
  if (length(candidates) == 0) {
    stop("No VM2Uniform parquet files found for ", state)
  }
...
```{warning}
Each state Parquet file contains the full ~800-column schema. Loading all columns for an entire state is memory-intensive and should not run on a KLC login node. Read only the columns you need, or submit a [KLC Reserve](../../klc-reserve/when-to-use) batch job for large extracts. You can also query the Parquet files with DuckDB on KLC without loading the full table into memory.
```
...
On Redivis, the L2 VM2 Uniform data is organized as **one table per state**, named using the two-letter state abbreviation followed by `_Uniform` — for example `AK_Uniform`, `AL_Uniform`, `AR_Uniform`, … `WY_Uniform`. On KLC, the same per-state tables are stored as Parquet files under `/kellogg/data/l2/parquet/VM2Uniform/{ST}/`, with one file per snapshot date. Each table has the same ~800-column schema. To work with multiple states on Redivis, use `UNION ALL` across the relevant state tables (see the [multi-state query example](#l2-queries) below).
