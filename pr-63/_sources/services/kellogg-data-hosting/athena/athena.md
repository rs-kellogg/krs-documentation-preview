...
The Kellogg Data Hosting **Athena** platform is built on **Amazon Athena**, a serverless SQL query service. You can query Athena datasets through the AWS web console or from KLC with Python and [awswrangler](https://aws-sdk-pandas.readthedocs.io/en/3.17.1/stubs/awswrangler.athena.read_sql_query.html). For R or Stata workflows on KLC, run the Python query, save the result to a file on the cluster, and read that file in your R or Stata script.
...
| [KLC](#accessing-via-klc) | Python queries with awswrangler; R and Stata read the saved query output |
...
Querying Athena from KLC uses Python and awswrangler. After you complete steps 1–3, run the Python example in step 4 to pull data and write a CSV on the cluster. If your analysis is in R or Stata, read that CSV in your script — do not connect to Athena directly from R or Stata.
...
```{warning}
Note the exact profile name the CLI prints at the end of setup. You will pass this name to `--profile` in step 3 and to the `profile` variable in the Python script in step 4.
```

```{tip}
If you forget a profile name, run `cat ~/.aws/config` on KLC. Each `[profile ...]` block is one configured dataset access. Match the block's `sso_role_name` (for example `ksm-rch-data-comscore2`) to your workgroup or dataset, then use the profile name from the section header in steps 3 and 4.
```
...
Replace `<account-profile>` with the profile name from step 2 (for example, `ksm-rch-data-comscore2-<account-id>`). If you do not have the name handy, run `cat ~/.aws/config` and use the profile from the matching `[profile ...]` section.

### 4. Query with Python

```{note}
**R and Stata on KLC:** We do not support connecting to Athena from R or Stata. Run the Python script below to query Athena and write a CSV on the cluster, then read that file in your R or Stata script (see [Using the Result in R or Stata](#using-the-result-in-r-or-stata)).
```

#### Install awswrangler

Create a conda environment in your home directory with [awswrangler](https://aws-sdk-pandas.readthedocs.io/en/3.17.1/stubs/awswrangler.athena.read_sql_query.html). Run the `eval` and `source` lines in every new shell before `mamba activate`:
...
mamba create --prefix=$HOME/aws-athena-env python awswrangler --yes
mamba activate $HOME/aws-athena-env
```

See [Conda Environments on KLC](/services/klc/user-guide/klc-conda) for general environment guidance.

#### Map script values to your access

Use the same workgroup and database you would select in the [Athena Query editor](#2-navigate-to-athena). Set `profile` to the exact name from step 2, or look it up with `cat ~/.aws/config` as described above.

| Script variable | Where to find it | Example (Fetch Rewards) |
|---|---|---|
| `workgroup` | Query editor, upper right; also the suffix after `ksm-rch-data-` in your SSO role | `fetchrewards` |
| `database` | Query editor, left panel (often different from the workgroup name) | `fetchrewards` |
| `profile` | Profile name from step 2; or `[profile ...]` in `~/.aws/config` matched via `sso_role_name` | `ksm-rch-data-fetchrewards-<account-id>` |
| `s3_output` | Built in the script as `s3://kdc-{workgroup}/queries/{workgroup}/unload/{uuid}` | `s3://kdc-fetchrewards/queries/fetchrewards/unload/...` |

#### Run a query

Save the script below as `athena_query.py`, replace the three values at the top, and run it from an activated environment:
...
# Values from the mapping table above.
workgroup = "fetchrewards"
database = "fetchrewards"
profile = "ksm-rch-data-fetchrewards-<account-id>"

sql = """
SELECT partner_id, barcode, cleaned_barcode
FROM barcode_lookup
LIMIT 10
"""

session = boto3.Session(profile_name=profile, region_name="us-east-2")
s3_bucket = f"kdc-{workgroup}"
s3_prefix = f"queries/{workgroup}/unload/{uuid.uuid4()}"
...
df = wr.athena.read_sql_query(
    ...
    athena_cache_settings={
        "max_cache_seconds": 1000,
        "max_cache_query_inspections": 100,
        "max_remote_cache_entries": 100,
        "max_local_cache_entries": 100,
    },
    s3_additional_kwargs={"region": "us-east-2"},
    boto3_session=session,
)

output_csv = "barcode_lookup_sample.csv"
df.to_csv(output_csv, index=False)

print(df.head())
print(df.query_metadata["Statistics"]["DataScannedInBytes"])
print(f"Wrote {len(df)} rows to {output_csv}")
```

The table name and columns match the Fetch Rewards `barcode_lookup` table. Use the Query editor or **Generate table DDL** to confirm names for other tables in your database.

```{note}
The script uses Athena **UNLOAD** to stage results in S3, then loads them into a pandas DataFrame. For large result sets, iterate with `chunksize` in `read_sql_query` and run the job on [KLC Reserve](../../klc-reserve/when-to-use), not a login node.
```

(using-the-result-in-r-or-stata)=
#### Using the Result in R or Stata

The Python script writes `barcode_lookup_sample.csv` in your current working directory. Change the `output_csv` variable in the script if you need a different path or filename. Then load the file in R or Stata:
...
```r
df <- read.csv("barcode_lookup_sample.csv")
```
...
```stata
import delimited "barcode_lookup_sample.csv", clear
```
...
:::{dropdown} AWS CLI commands fail with an authentication or token error

If AWS CLI commands fail with an authentication or token error, your SSO session has expired. Run:
...
:::{dropdown} A Python awswrangler query fails with an authentication or permission error

If `athena_query.py` fails with an authentication or token error, your SSO session has expired. Run:

```bash
aws sso login --sso-session nu-sso --no-browser --use-device-code
```

If authentication succeeds for `aws s3 ls` but the Python script still fails, confirm `profile` in the script matches the profile name from step 2 exactly. Confirm `workgroup` and `database` match the Query editor — the database name is often different from the workgroup name.
