...
For CPU vs. GPU concepts and when a GPU helps, see [GPU Concepts and Options](/services/klc-reserve/gpu-concepts). For non-GPU batch jobs, see [Submitting SLURM Jobs](slurm-jobs). For host-RAM-bound CPU work on the dedicated high-memory node, see [High-Memory Jobs](highmem-jobs).
...
- [High-Memory Jobs](highmem-jobs) — host-RAM-bound CPU work on `qhimem0501`
