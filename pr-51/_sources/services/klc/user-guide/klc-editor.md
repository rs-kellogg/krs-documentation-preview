...
* `tail filename` – Shows the **last 10 lines** of a file. Great for checking logs. Use `tail -f filename` to follow a log file in real time as it grows. See [Launching Jobs on KLC](klc-software) for how to create log files when running scripts interactively.
