...
- Optional: R with `httr` and `jsonlite` installed if you want to run the R example near the end

## Materials

Download the tutorial files, unzip, and run the Python scripts from the `code/` folder (they load paragraphs from the sibling `data/groups/` folder). The same zip file also includes a stripped-down R example in `scripts_r/`, with a `README.md` that gives more detailed setup and run instructions:

- {download}`Tutorial files (zip) <qual-coding-materials.zip>` — five Python scripts, six sample paragraphs, the R skeleton scripts, sample R input files, and an example output CSV
...
## 10. R Skeleton: A Minimal Resumable Pipeline

The Python examples above show the full qualitative-coding workflow for the security-force toy example: rubric design, ambiguity handling, calibration, scaling, reliability checks, and adjudication. The download zip also includes a stripped-down R version in `scripts_r/`. It does not include images, model comparison, or reliability statistics. Its purpose is narrower: show the basic mechanics of sending rows to an LLM, saving each result as it returns, and making the run safely resumable.

The R code lives in `scripts_r/skeleton_qual_coding.R` and `scripts_r/skeleton_qual_coding.Rmd`. The script reads `data/system.txt`, `data/sample_input.csv`, and the local key file at `scripts_r/key.txt`, writes to `outputs/sample_coded.csv`, and logs the model and timestamp that produced each coded row. See `scripts_r/README.md` in the zip file for package installation, API key setup, and command-line instructions.

### 10.1 Setup

Start by loading the system prompt, choosing a model, reading the API key from `scripts_r/key.txt`, reading the sample data, and setting an output path. The downloaded zip includes a blank `scripts_r/key.txt` template. The key file should contain `OPENROUTER_API_KEY=sk-or-your-key-here`. Double quotes are not needed after the equals sign, though the script will tolerate them if they are included.
...
# seed is best-effort reproducibility. Some models honor it, others ignore it.
temperature <- 0
seed <- 42

read_openrouter_key <- function(path = "scripts_r/key.txt") {
  if (!file.exists(path)) {
    stop("Could not find scripts_r/key.txt. Create that file and add: OPENROUTER_API_KEY=your-key-here")
  }

  lines <- trimws(readLines(path, warn = FALSE))
  lines <- lines[nzchar(lines) & !startsWith(lines, "#")]
  if (length(lines) == 0) {
    stop("scripts_r/key.txt is empty. Add: OPENROUTER_API_KEY=your-key-here")
  }

  entry <- lines[1]
  if (grepl("=", entry, fixed = TRUE)) {
    parts <- strsplit(entry, "=", fixed = TRUE)[[1]]
    key_name <- trimws(parts[1])
    api_key <- trimws(paste(parts[-1], collapse = "="))
    if (!identical(key_name, "OPENROUTER_API_KEY")) {
      stop("In scripts_r/key.txt, use the name OPENROUTER_API_KEY before the equals sign.")
    }
  } else {
    api_key <- entry
  }
  ...
  if (nchar(api_key) >= 2) {
    first_char <- substr(api_key, 1, 1)
    last_char <- substr(api_key, nchar(api_key), nchar(api_key))
    if ((first_char == "\"" && last_char == "\"") || (first_char == "'" && last_char == "'")) {
      api_key <- substr(api_key, 2, nchar(api_key) - 1)
    }
  }

  if (identical(api_key, "")) {
    stop("OPENROUTER_API_KEY in scripts_r/key.txt is blank. Paste your key after the equals sign.")
  }
...
The R skeleton treats `data/sample_input.csv` as the authoritative list of rows. If `outputs/sample_coded.csv` already exists from an earlier run, the script merges prior `llm_code`, `model_id`, and `coded_at` values back in by `id`, so only new or missing rows get coded.
...
### 10.2 Preview the Prompt

Before spending API credits, print the shared system prompt and one sample user message. This is the same practical habit used throughout the Python walkthrough: inspect exactly what the model will see before scaling up.
...
For the skeleton data, the preview shows a workplace-collaboration coding task rather than the security-force rubric used earlier in the tutorial:

```text
===== SYSTEM PROMPT (sent once, shared by every item) =====

You are a qualitative coding assistant helping a research team analyze open-ended survey responses about workplace collaboration challenges.

Task: Classify the response below into exactly ONE of the following categories. Do not invent a new category.

Categories:
1. Communication Breakdown: Misunderstandings, unclear messaging, or lack of information sharing between team members.
2. Resource Constraints: Insufficient time, staffing, budget, or tools to complete collaborative work.
3. Role Ambiguity: Confusion about who is responsible for what, or overlapping/unclear responsibilities.
4. Leadership Gaps: Lack of direction, decision-making, or support from managers or team leads.
5. Positive Collaboration: Effective teamwork, mutual support, or successful coordination, with no significant challenge described.

Return only the category name exactly as written above. Do not include numbering, punctuation, explanation, or any other text.

===== SAMPLE USER MESSAGE (row 1) =====

We often find out about schedule changes secondhand, and by the time everyone's on the same page a full day has been lost.
```

### 10.3 Code One Item

The API call follows the same system-prompt/user-prompt split introduced in Section 2. The system prompt holds the coding instructions, while the user message is the text from one row of `sample_input.csv`.
...
For production use, you would usually add a validation step after this function returns, checking that the model output exactly matches one of the allowed categories in the codebook.

### 10.4 Code Every Row

Finally, loop over the rows, skip anything already coded, and write the CSV after each response. Saving after every row is intentionally cautious: if the script is interrupted halfway through, the next run can resume from the last completed item.

```r
for (i in seq_len(nrow(input_data))) {
  if (!is.na(input_data$llm_code[i])) {
    next # already coded in a previous run
  }
...
### 10.5 What to Change for Your Own Task

- **`data/system.txt`** — replace the workplace-collaboration prompt with your own background, category definitions, and output rules.
- **`data/sample_input.csv`** — replace the sample rows with your data; keep an `id` column for merging and a `text` column for the user message.
- **`model_id`** — choose a model supported by your API provider, and check which parameters it supports.
- **`Sys.sleep(1)`** — adjust the pause between calls to match your rate limit.
