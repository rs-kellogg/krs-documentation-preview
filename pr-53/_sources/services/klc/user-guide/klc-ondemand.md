...
KLC OnDemand is a browser-based interface for the Kellogg Linux Cluster, accessible through [Quest OnDemand](https://ondemand.quest.northwestern.edu) and built on the [Open OnDemand](https://openondemand.org/) platform. It lets you launch graphical applications, development environments, and a file manager directly in your browser. No SSH client or local software installation is required.
...
The same tables are available without logging in on [KLC Node Resources](/services/klc-node-resources).
...
3. **Working Directory** *(optional)* — The folder VS Code will open on launch. Leave this field blank to start from your home directory. To open a specific Kellogg project directory, enter the full path here (e.g., `/kellogg/proj/your-netid/my-project`).
