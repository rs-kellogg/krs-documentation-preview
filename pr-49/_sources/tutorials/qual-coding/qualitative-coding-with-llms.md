...
The R code lives in `scripts_r/skeleton_qual_coding.R` and `scripts_r/skeleton_qual_coding.Rmd`. The script reads `data/system.txt` and `data/sample_input.csv`, writes to `outputs/sample_coded.csv`, and logs the model and timestamp that produced each coded row. See `scripts_r/README.md` in the zip file for package installation, API key setup, and command-line instructions.

### 10.1 Setup

Start by loading the system prompt, choosing a model, reading the sample data, and setting an output path. The script expects an OpenRouter API key in the `OPENROUTER_API_KEY` environment variable, so the key never has to be hard-coded in the file.
...
if (identical(api_key, "")) {
  stop("OPENROUTER_API_KEY is not set. Set it before running the script.")
}
