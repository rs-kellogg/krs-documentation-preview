...
```{note}
We list EDGAR here because Kellogg Research Support maintains a pre-downloaded copy on KLC at `/kellogg/data/EDGAR/`, organized by form type and year, so researchers can run large-scale analysis without downloading filings from the SEC themselves.
```
