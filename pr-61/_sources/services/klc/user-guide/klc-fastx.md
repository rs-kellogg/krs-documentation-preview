...
Each URL connects to a specific KLC node. Check the **KLC Direct Access Resource Availability** table on [KLC Node Resources](/services/klc-node-resources) and pick a node with available CPU and RAM before launching FastX on that host. The same table appears on the [KLC OnDemand](klc-ondemand) landing page after you log in.
