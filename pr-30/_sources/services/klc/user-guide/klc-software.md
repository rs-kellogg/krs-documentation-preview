...
If your job will run for many hours, needs more than 24 CPU cores, or you want to run many similar jobs in parallel, submit through **KLC Reserve** instead of running directly on a login node. SLURM reserves resources exclusively for your job and runs it whether you are logged in or not. See [When to Use KLC Reserve](/services/klc-reserve/when-to-use) for when to switch, and [Submitting SLURM Jobs](/services/klc-reserve/slurm-jobs) for how-to instructions.
