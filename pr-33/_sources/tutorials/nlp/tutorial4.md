...
When the server is ready, the script prints the node hostname and port. Use those values for API calls. For GPU workloads, submit the same script with `sbatch` — see [Open Source LLMs on KLC](/services/klc/user-guide/llm-klc).

Verify the server is running (replace `<PORT>` with the port from the ready banner):
...
| **Setup** | Start the Ollama launcher script on KLC | Requires API key |
