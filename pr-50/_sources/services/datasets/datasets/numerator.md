...
## Example Redivis SQL Queries

These queries are designed to run in Redivis using the current v3.x schema. They demonstrate common item-level and basket-level analyses and do not require `BATCH_DATE` filtering or deduplication.

### Monthly Item-Level Spend By Channel

Use `itemlvl_fact_table` when the analysis depends on item, product, brand, manufacturer, department, or category fields.
...
SELECT
    ...
    SUM(f.ITEM_TOTAL) AS item_spend
FROM standard_nmr_feed_itemlvl_fact_table AS f
JOIN standard_nmr_feed_banner_table AS b
    ON f.BANNER_ID = b.BANNER_ID
JOIN standard_nmr_feed_item_table AS i
    ON f.ITEM_ID = i.ITEM_ID
WHERE i.DEPARTMENT_DESCRIPTION = 'Beverages'
...
### Monthly Basket-Level Spend By Retailer

Use `summarylvl_fact_table` when the analysis needs the broadest basket coverage and does not require item-level categories.

```sql
SELECT
    b.RETAILER_ID,
    b.RETAILER,
    DATE_TRUNC(s.TRANSACTION_DATE, MONTH) AS month,
    COUNT(DISTINCT s.BASKET_ID) AS trips,
    SUM(s.BASKET_ADJUSTED_SUBTOTAL) AS adjusted_spend
FROM standard_nmr_feed_summarylvl_fact_table AS s
JOIN standard_nmr_feed_banner_table AS b
    ON s.BANNER_ID = b.BANNER_ID
WHERE s.TRANSACTION_DATE >= DATE '2025-01-01'
  AND s.TRANSACTION_DATE <  DATE '2026-01-01'
GROUP BY 1, 2, 3
ORDER BY 1, 3
LIMIT 10;
```

### Daily Price Distribution For A Product Category

This example adapts an older workshop query for egg prices to the current Redivis v3.x schema. It uses `itemlvl_fact_table` because price distributions require item-level product records. The older workshop version used `standard_nmr_feed_fact_table` and `BATCH_DATE` filters; those are not needed for current v3.x data.
...
SELECT
    ...
    COUNT(*) AS item_rows,
    COUNT(DISTINCT f.BASKET_ID) AS trips,
    AVG(f.ITEM_UNIT_PRICE) AS avg_unit_price,
    STDDEV(f.ITEM_UNIT_PRICE) AS std_unit_price,
    APPROX_QUANTILES(f.ITEM_UNIT_PRICE, 100)[OFFSET(24)] AS p25_unit_price,
    APPROX_QUANTILES(f.ITEM_UNIT_PRICE, 100)[OFFSET(49)] AS median_unit_price,
    APPROX_QUANTILES(f.ITEM_UNIT_PRICE, 100)[OFFSET(74)] AS p75_unit_price
FROM standard_nmr_feed_itemlvl_fact_table AS f
JOIN standard_nmr_feed_item_table AS i
    ON f.ITEM_ID = i.ITEM_ID
...
```{note}
`ITEM_UNIT_PRICE` is useful for within-category price distributions, but researchers should still inspect outliers before interpreting daily averages. Receipt text can also be noisy, so filters such as `LIKE '%12%'` are convenient starting points rather than final product definitions.
```
