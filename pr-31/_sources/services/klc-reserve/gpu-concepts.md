...
Smaller open-source models run on KLC CPU login nodes via [Ollama](/services/klc/user-guide/llm-klc). Larger models and batch inference benefit from Reserve GPU nodes. See [Open Source LLMs Guide](/guides/llm/llm) for the broader comparison with hosted APIs.
