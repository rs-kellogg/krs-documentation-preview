...
For a worked example that serves a model with vLLM in Singularity and queries it via the OpenAI-compatible API, see [vLLM Inference on KLC Reserve GPUs](vllm-example).
...
Researchers who also hold a Quest allocation can use the **`gengpu`** partition on Quest GPU nodes. That path requires a separate [Quest allocation](https://www.it.northwestern.edu/departments/it-services-support/research/computing/quest/general-access-allocation-types.html). Kellogg-affiliated researchers typically use **`kellogg`** first. See [GPU Concepts and Options](/services/klc-reserve/gpu-concepts) for the comparison.
